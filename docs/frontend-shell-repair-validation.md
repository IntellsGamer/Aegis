...
A separate native Waitress instance on port 8011 successfully served the authenticated dashboard. The profile route then hydrated its saved account and preference controls correctly through that WSGI instance before the rapid navigation check.

The first rapid profile-to-dashboard return revealed an independent Chart.js cache issue (`Canvas is already in use`) caused by Turbo retaining canvas instances. The dashboard now destroys a cached Chart.js instance before drawing either graph. A repeat dashboard → profile → dashboard transition completed without either the chart-canvas error or the profile null-field error. The final browser console was clean after replacing Turbo’s deprecated progress-delay API; the active `Turbo.config.drive.progressBarDelay` was verified at 80ms, and the two-pixel top progress-bar rule is present in the locally served stylesheet.
