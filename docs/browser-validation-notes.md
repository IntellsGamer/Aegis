...
## Exhaustive Persian UI audit

Profile audit passed: the account, privacy, notification, password, and danger-zone controls all rendered in Persian. During the locale transition the readiness guard produced a blank pre-ready frame rather than English text in an RTL shell; the page content extracted after reload was fully localized.
Dashboard audit found significant untranslated interface strings: heading, greeting, New Scan, stored-scan and threat statistics, review wording, recent-scan labels, chart titles, and type labels. Scanner audit passed for its static user-interface chrome; any future English evidence titles or recommendations are source evidence rather than untranslated controls.
Map audit found its complete interface untranslated: community-intelligence heading, range controls, provenance, empty state, severity legend, and aggregate cards. Learning-center audit found its lesson, quiz, simulator, filters, labels, and progress surface untranslated. Both require full page and client-string localization rather than isolated label fixes.
Admin audit found untranslated platform analytics, all navigation tabs, threat-intelligence controls, and dynamic table actions. Search audit found untranslated heading, description, scope options, placeholder, and submit control. These are genuine interface-localization defects, not source evidence, and will be fixed in the next implementation pass.
Casefile audit found its heading, export actions, assessment narrative, evidence-chain metadata, response playbook, scope/provenance, and limitation strings untranslated. Home audit found the complete public value proposition and action surface untranslated. These pages require localized product narrative and dynamic casefile labels while preserving raw source evidence and identifiers unchanged.
Unauthenticated audit found sign-in and registration completely English-only. Because a user has no stored locale before authentication, full Persian support here requires a public locale choice or browser-language default, not merely the authenticated header preference.
Password-recovery audit found both reset-request and set-new-password pages English-only. This completes the public access finding: a persistent locale cookie or an unauthenticated header/selector is required to localize login, registration, and recovery before a user account exists.
The public sign-in retest passed after the locale-cookie addition: the unauthenticated header selector chose Persian, persisted it across reload, rendered `lang=fa`/RTL, and translated the sign-in interface. This closes the earlier pre-login locale-selection gap.
The settled Persian public sign-in page passed visual inspection: language selector, heading, labels, recovery link, remember-me control, account link, submit button, footer, Vazirmatn typography, and RTL layout all rendered correctly.
Dashboard retest passed after remediation: personalized greeting, metrics, review notes, scan-type labels, chart headings, and chart legends now render in Persian. The remaining English within scan cards is raw target/email evidence and is intentionally preserved rather than translated or altered.
Map remediation retest passed: dynamic report-count status and empty-state explanation now render in Persian. OpenStreetMap and Leaflet names/attribution remain in their original legal form as third-party attribution, not Aegis interface copy.
