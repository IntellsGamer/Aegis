(function () {
  ...
  const translations = {
    fa: {
      ...
      'scan.feedback_desc': 'بازخورد شما جدا از موتور ثبت می‌شود و موتور را خودکار آموزش نمی‌دهد. اگر این مورد را کلاه‌برداری تأیید کنید، برای بررسی تحلیل‌گر هم در صف قرار می‌گیرد.',
      'scan.triage_queued': 'بازخورد ثبت و برای بررسی تحلیل‌گر صف‌بندی شد. فقط پس از تأیید تحلیل‌گر، در اطلاعات تهدید و نقشهٔ تجمیعیِ کشوری دیده می‌شود.',
      'scan.triage_queued_no_location': 'بازخورد ثبت و برای بررسی تحلیل‌گر صف‌بندی شد. موقعیت کشوریِ قابل‌اعتمادی برای این بررسی ثبت نشده است؛ پس حتی پس از تأیید هم روی نقشهٔ کشوری نمایش داده نمی‌شود.',
      'scan.triage_queued_development': 'بازخورد ثبت و برای بررسی تحلیل‌گر صف‌بندی شد. پس از تأیید، نقشهٔ محلیِ محیط توسعه از کشور تنظیم‌شده به‌عنوان لنگر آزمایشی استفاده می‌کند؛ این داده هرگز اطلاعات عمومی تهدید نیست.',
      ...
      'scan.progress_start': 'در حال آماده‌سازی بررسی…',
      'scan.progress_remote': 'بررسی امن مقصد و گواهی رمزنگاری…',
      'scan.progress_extended': 'برخی بررسی‌های راه‌دور بیشتر زمان می‌برند؛ نتیجه یا محدودیت پوشش نمایش داده می‌شود.',
      ...
      'scan.share_ready': 'محتوای ارسال‌شده آمادهٔ بررسی است. هر زمان آماده بودید، تحلیل را شروع کنید.',
      'scan.limited_title': 'بررسی با اطلاعات محدود',
      'scan.limited_body': 'بخشی از بررسی‌های راه‌دور کامل نشد. این وضعیت نه خطرناک بودن را ثابت می‌کند و نه بی‌خطر بودن را؛ فقط پوشش شواهد را محدود می‌کند.',
      ...
      'admin.triage_title': 'صف تریاژ تحلیل‌گر',
      'admin.triage_description': 'ارزیابی‌های پرخطر و گزارش‌هایی که کاربر بدخواه‌بودنشان را تأیید کرده است، برای بررسی انسانی مرتب می‌شوند. ثبت نتیجه از امتیازدهی جداست: موتور را دوباره آموزش نمی‌دهد، شواهد را تغییر نمی‌دهد و اطلاعات تهدید را خودکار منتشر نمی‌کند.',
      'admin.triage_purpose': 'صف بررسی انسانی برای ارزیابی‌های پرخطر و گزارش‌های بدخواهِ تأییدشده توسط کاربر. مواردی که فقط به‌دلیل مرز ایمنی مسدود شده‌اند کنار گذاشته می‌شوند؛ نتیجهٔ بررسی از امتیاز موتور جداست.',
      ...
      'report.summary_safe': 'برای این {type} نشانهٔ پرخطر مشخصی دیده نشد.', 'report.summary_medium': 'این {type} چند نشانهٔ هشدار دارد.', 'report.summary_high': 'این {type} نشانه‌های قوی از کلاه‌برداری دارد.', 'report.summary_critical': 'احتمال کلاه‌برداری‌بودن این {type} بسیار بالاست.',
      'report.action_low_evidence': 'نشانهٔ پرخطر مشخصی دیده نشد؛ بااین‌حال این نتیجه تضمین ایمنی نیست. پیش از ارائهٔ اطلاعات حساس، فرستنده یا مقصد را از مسیر مستقل بررسی کنید.', 'report.action_low_hardening': 'برای حساب‌های مهم از مدیر گذرواژه و احراز هویت چندمرحله‌ای استفاده کنید.',
      'report.action_unverified': 'این مورد را تأییدنشده در نظر بگیرید. برای راستی‌آزمایی از پیوند، شماره تلفن یا اطلاعات تماسی که خود پیام داده است استفاده نکنید.', 'report.action_verify_official': 'درخواست را از طریق وب‌سایت یا برنامهٔ رسمیِ شناخته‌شده، یا یک کانال تماس مستقل بررسی کنید.',
      'report.action_shared_info': 'اگر اطلاعاتی را قبلاً به اشتراک گذاشته‌اید، اعتبارنامه‌های درگیر را تغییر دهید و فوراً با سرویس اصلی تماس بگیرید.',
      'report.highlight_no_high_risk': 'نشانهٔ پرخطر مشخصی دیده نشد', 'report.highlight_limited_confidence': 'اطمینان ارزیابی به شواهد در دسترس محدود است',
      'report.title_https': 'HTTPS فعال است', 'report.title_ssl_valid': 'گواهی SSL معتبر است', 'report.title_tls_limited': 'بررسی گواهی TLS کامل نشد', 'report.title_domain_very_old': 'دامنه سابقهٔ طولانی دارد', 'report.title_domain_old': 'دامنه سابقه‌دار است', 'report.title_no_scam_patterns': 'الگوی کلاه‌برداری شناخته‌شده‌ای دیده نشد',
      'report.desc_https': 'وب‌سایت با اتصال رمزنگاری‌شدهٔ HTTPS ارائه می‌شود.', 'report.desc_ssl_valid': 'گواهی SSL این نشانی در حال حاضر معتبر است.', 'report.desc_tls_limited': 'AEGIS نتوانست بررسی گواهی TLS این مقصد را کامل کند. این یک محدودیت پوشش است، نه نشانهٔ تهدید.', 'report.desc_domain_very_old': 'این دامنه بیش از پنج سال پیش ثبت شده است؛ این یک نشانهٔ مثبت اعتماد است.', 'report.desc_domain_old': 'این دامنه بیش از یک سال سابقهٔ ثبت دارد.', 'report.desc_no_scam_patterns': 'در شواهد گردآوری‌شده الگوی شناخته‌شده‌ای از کلاه‌برداری پیدا نشد.',
      'report.source_url_observation': 'بررسی نشانی وب', 'report.source_tls_observation': 'بررسی رمزنگاری', 'report.source_content_semantics': 'تحلیل محتوای پیام', 'report.source_page_or_request': 'بررسی صفحه یا درخواست', 'report.source_heuristic': 'قاعدهٔ تحلیلی',
      'report.title_persian_legal_case': 'فریب فشار با ادعای پروندهٔ قضایی', 'report.title_persian_legal_attachment': 'فریب پیوست و رهگیریِ پروندهٔ قضایی', 'report.title_persian_familiar_transfer': 'فریب انتقال پول به نام آشنا', 'report.title_persian_deferred_repayment': 'فریب وعدهٔ بازپرداختِ بعدی',
      'report.title_persian_invoice': 'فریب فشار برای پرداخت صورت‌حساب', 'report.title_persian_executive_transfer': 'فریب انتقال محرمانه به نام مدیر', 'report.title_persian_support_callback': 'فریب تماس با پشتیبانی جعلی', 'report.title_persian_investment_return': 'فریب بازده سرمایه‌گذاری غیرعادی',
      'report.title_persian_reward_fee': 'فریب جایزه با هزینهٔ پیش‌پرداخت', 'report.title_persian_job_document': 'فریب پیشنهاد شغلی و درخواست مدارک', 'report.title_persian_service_cutoff': 'فریب قطع سرویس و درخواست پرداخت', 'report.title_persian_bank_lockout': 'فریب مسدودی حساب بانکی',
      'report.title_persian_stranded_friend': 'فریب دوستِ گرفتار و درخواست انتقال پول', 'report.title_persian_beta_payment': 'فریب دسترسی آزمایشی و درخواست اطلاعات پرداخت',
      'report.desc_persian_legal_case': 'ادعای تشکیل پرونده علیه مخاطب با عبارت فشارآورِ «هشدار نهایی» همراه شده است.', 'report.desc_persian_legal_attachment': 'ادعای پروندهٔ قضایی، مخاطب را به رهگیری یا باز کردن پیوست هدایت می‌کند؛ این شیوه در ارسال‌های فریبنده رایج است.',
      'report.desc_persian_familiar_transfer': 'ادعای پرشدن سقف کارت برای درخواست انتقال پول به شخص دیگر به‌کار رفته است.', 'report.desc_persian_deferred_repayment': 'درخواست انتقال پول با وعدهٔ بازپرداخت در زمانی دیگر همراه شده است؛ این بهانه در جعل هویت رایج است.',
      'report.desc_persian_invoice': 'ادعای صورت‌حساب معوق با مطالبهٔ پرداخت فوری یا فشار حقوقی همراه شده است.', 'report.desc_persian_executive_transfer': 'فردی با ادعای سمت مدیریتی، انتقال محرمانهٔ پول را بدون امکان راستی‌آزمایی مستقل درخواست می‌کند.',
      'report.desc_persian_support_callback': 'هشدار فنی یا امنیتی، مخاطب را به تماس با شمارهٔ پشتیبانیِ تأییدنشده هدایت می‌کند.', 'report.desc_persian_investment_return': 'پیشنهاد سرمایه‌گذاری با ادعای سود تکرارشونده یا تضمینی، سرمایه درخواست می‌کند.',
      'report.desc_persian_reward_fee': 'ادعای جایزه یا پاداش، پرداخت هزینهٔ اداری یا پردازش را پیش از دریافت آن لازم می‌داند.', 'report.desc_persian_job_document': 'پیشنهاد شغلی پیش از طی‌کردن فرایند استخدامِ قابل‌تأیید، مدارک هویتی یا بانکی درخواست می‌کند.',
      'report.desc_persian_service_cutoff': 'اعلان قطع برق، تلفن، اینترنت یا بیمه، مخاطب را به پرداخت یا اقدام فوری روی پیوند هدایت می‌کند.', 'report.desc_persian_bank_lockout': 'ادعای مسدودی حساب بانکی، احراز هویت را از طریق پیوند یا اطلاعات تماسِ داخل پیام درخواست می‌کند.',
      'report.desc_persian_stranded_friend': 'ادعای گرفتارشدن یک آشنا در خارج از کشور با درخواست انتقال پول و وعدهٔ بازپرداخت همراه شده است.', 'report.desc_persian_beta_payment': 'دعوت به دسترسی آزمایشی، پیش از ارائهٔ دسترسی اطلاعات پرداخت درخواست می‌کند.',
  ...
  Object.assign(literalTranslations.fa, {
    ...
    'Human review workflow': 'فرایند بررسی انسانی', 'Analyst triage queue': 'صف تریاژ تحلیل‌گر', 'Open casefile': 'باز کردن پرونده', 'Evidence families': 'خانواده‌های شواهد', 'No family summary available.': 'خلاصهٔ خانوادهٔ شواهد در دسترس نیست.', 'Strongest recorded evidence': 'قوی‌ترین شواهد ثبت‌شده', 'No evidence summary available.': 'خلاصهٔ شواهد در دسترس نیست.', 'Latest review:': 'آخرین بررسی:', 'Confirm malicious': 'تأیید بدخواه بودن', 'Confirm benign': 'تأیید بی‌خطر بودن', 'Mark inconclusive': 'علامت‌گذاری نامشخص', 'A latest review is recorded. Use the casefile and audit log for follow-up.': 'آخرین بررسی ثبت شده است. برای پیگیری از پرونده و گزارش حسابرسی استفاده کنید.', 'No high-risk persisted assessments are awaiting operational triage.': 'هیچ ارزیابی پرخطر ذخیره‌شده‌ای در انتظار تریاژ عملیاتی نیست.', 'Pending community report': 'گزارش کاربریِ در انتظار بررسی', 'Approval publishes the reviewed indicator and, when a country is available, the approved country aggregate.': 'تأیید، شاخصِ بازبینی‌شده را منتشر می‌کند و در صورت وجود کشور، تجمیع کشوریِ تأییدشده را نیز نمایش می‌دهد.', 'Approve & publish': 'تأیید و انتشار', 'Reject report': 'رد گزارش', 'Report approved and published': 'گزارش تأیید و منتشر شد', 'Report rejected': 'گزارش رد شد', 'No country anchor': 'بدون لنگر کشوری',
