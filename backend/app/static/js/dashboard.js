...
(function () {
  ...
  const statCards = [
    { key: 'scans', label: 'Stored scans' },
    { key: 'threats', label: 'Threats detected' },
    { key: 'avg_score', label: 'Average trust score' },
  ...
  function renderScans(scans) {
    ...
    box.innerHTML = scans.slice(0, 5).map((scan) => `<a href="/report/${scan.id}" class="flex items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3 transition hover:border-aegis-500/50 dark:border-slate-800 dark:bg-slate-900/50"><div class="min-w-0"><p class="truncate text-sm font-medium">${esc(scan.target)}</p><p class="text-xs text-slate-500">${esc(scan.scan_type)} · ${fmtTime(scan.created_at)}</p></div><div class="flex shrink-0 items-center gap-2"><span class="font-mono text-sm">${scan.trust_score ?? '—'}</span>${badgeFor(scan.verdict)}</div></a>`).join('');
  }
  ...
  function drawCharts(scans) {
    ...
    if (activity) new Chart(activity, { type: 'line', data: { labels, datasets: [{ label: 'Scans', data: days.map((date) => counts[date] || 0), borderColor: '#06b6d4', backgroundColor: 'rgba(6,182,212,.15)', fill: true, tension: 0.3, pointRadius: 2 }] }, options: { ...common, scales: { y: { beginAtZero: true, ticks: { precision: 0 } } } } });
    const score = document.getElementById('score-canvas');
    if (score) new Chart(score, { type: 'line', data: { labels, datasets: [{ label: 'Trust', data: days.map((date) => scores[date] ?? null), borderColor: '#10b981', pointRadius: 3, tension: 0.3, spanGaps: true }] }, options: { ...common, scales: { y: { min: 0, max: 100 } } } });
  }
